...
This document outlines the steps and configurations required to deploy the Astro website to GitHub Pages, including using a custom domain (e.g., `subdomain.yourdomain.com`).
...
6.  **Deploy to gh-pages:** The `peaceiris/actions-gh-pages@v3` action is used to:
    ...
    *   Create a `CNAME` file in the `gh-pages` branch with the value `your.custom.domain.com`.
    ...
    steps:
      ...
      - name: Deploy 🚀
        ...
        with:
          ...
          cname: your.custom.domain.com # Custom domain
```
...
*   **Branch:**
    *   Branch: `gh-pages`
    *   Folder: `/ (root)`
*   **Custom Domain:** Ensure `your.custom.domain.com` (or your chosen domain) is entered and saved here. GitHub will attempt to verify your DNS settings.

## 3. DNS Configuration for `your.custom.domain.com`

To correctly point your custom subdomain (e.g., `subdomain.yourdomain.com`) to your GitHub Pages site, you need to configure DNS records with your DNS provider (e.g., GoDaddy, Namecheap, Cloudflare).

*   **For the subdomain (e.g., `subdomain.yourdomain.com`):**
    *   **Type:** `CNAME`
    *   **Host/Name:** `subdomain` (or `subdomain.yourdomain.com` depending on your provider's interface)
    *   **Value/Points to/Target:** `[your-username].github.io` (replace `[your-username]` with your GitHub username or organization name)

*   **Important:**
    *   **Do NOT use `A` records** (pointing to GitHub's IP addresses) for subdomains like `subdomain.yourdomain.com` when using GitHub Pages. This can lead to errors like `InvalidARecordError`.
...
*   **Custom Domain Not Working / DNS Errors (`InvalidARecordError`):**
    *   Double-check your DNS records with your provider. Ensure you are using a `CNAME` record for `your.custom.domain.com` pointing to `[your-username].github.io`.
    *   Allow time for DNS propagation.
    *   Verify the custom domain is correctly entered in your repository's GitHub Pages settings.

By following these steps, your Astro site should build and deploy automatically to `your.custom.domain.com` via GitHub Pages whenever you push changes to the `main` branch. to GitHub Pages
...
If you want to use a custom domain (e.g., `subdomain.yourdomain.com`):
...
*   **For a subdomain (e.g., `subdomain.yourdomain.com` or `www.yourdomain.com`):**
    Create a `CNAME` record pointing to `[your-username].github.io` (replace `[your-username]` with your GitHub username).
    Example: `CNAME subdomain -> [your-username].github.io.`
...
2.  Under "Custom domain", enter your custom domain (e.g., `subdomain.yourdomain.com`) and click **Save**.
